...
def parse_module_defs_user(module_defs):
    ...
    Conv_together = list(set(CBL_idx).union(set(Conv_idx)))
    prune_idx = [idx for idx in Conv_together if idx not in ignore_idx]

    return CBL_idx, Conv_idx, prune_idx

def parse_module_defs_userconv(module_defs):
    ...
    for i, module_def in enumerate(module_defs):
        if module_def['type'] == 'convolutional':
            if module_def['batch_normalize'] == '1':
                ...
                Conv_idx.append(i)
    Conv_together = list(set(CBL_idx).union(set(Conv_idx)))
    #prune_idx = [idx for idx in Conv_together]
    return CBL_idx, Conv_idx, Conv_together
...
def parse_module_defs0(module_defs):
    ...
    for i, module_def in enumerate(module_defs):
        if module_def['type'] == 'convolutional':
            ...
            if module_defs[identity_idx]['type'] == 'convolutional':
                ...
                ignore_idx.add(identity_idx - 1)
    prune_idx0 = [idx for idx in Conv_idx if idx not in ignore_idx]

    return Conv_idx, prune_idx0
...
def gather_conv_weights0(module_list, Conv_together, group_num):
    size_list = []
    conv_weights_list = []
    pruned_idx = []
    for idx in Conv_together:
        size = module_list[idx][0].weight.data.shape[1]
        weight = module_list[idx][0].weight.data
        if size > group_num:
            size_list.append(size)
            pruned_idx.append(idx)
            #conv_weights = torch.zeros(sum(size_list))
            conv_weights = weight.cpu().abs().detach().numpy()[:,:,:,:]
            ...
            conv_weights_list.append(conv_weights)
    print('cut all {} conv layers'.format(len(pruned_idx)))

    return conv_weights_list, pruned_idx
...
def obtain_quantiles(bn_weights, num_quantile=5):
    ...
    print("\nBN weights quantile:")
    quantile_table = [
        ['{i}/{num_quantile}' for i in range(1, num_quantile+1)],
...
def init_weights_from_loose_model_user(compact_model, loose_model, prune_idx, CBLidx2mask):
...
def prune_model_keep_size2_user(model, prune_idx, CBLidx2mask):
    pruned_model = deepcopy(model)
    for i, model_def in enumerate(model.module_defs):
        if model_def['type'] == 'convolutional':
            if i in prune_idx:

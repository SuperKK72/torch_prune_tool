...
if __name__ == '__main__':
    ...
    print('Global Threshold should be less than %.4f'%thresh)
    ...
    def obtain_filters_mask(model, thre, CBL_idx, prune_idx):
        ...
        for idx in CBL_idx:
            ...
            if idx in prune_idx:
                ...
                #print('layer index: {idx:>3d} \t total channel: {mask.shape[0]:>4d} \t '
                        #'remaining channel: {remain:>4d}')
                print('''layer index: %.3d \t total channel: %.4d \t remaining channel: %.4d'''%(idx,mask.shape[0],remain))
    ...
    def prune_and_eval(model, CBL_idx, CBLidx2mask):
        ...
        print('mask the gamma as zero, mAP of the model is %.4f'%mAP)
    ...
    def prune_and_eval2(model, prune_shortcuts=[]):
        ...
        print('simply mask the BN Gama of to_be_pruned CBL as zero, now the mAP is %.4f'%mAP)
    ...
    #metric_table = [
        ...
        #["mAP", '{origin_model_metric[0][2]:.6f}', '{compact_model_metric1[0][2]:.6f}', '{compact_model_metric2[0][2]:.6f}'],
        #["Parameters", "{origin_nparameters}", "{compact_nparameters1}", "{compact_nparameters2}"],
        #["Inference", '{pruned_forward_time:.4f}', '{compact_forward_time1:.4f}', '{compact_forward_time2:.4f}']
    #]
    metric_table = [
        ["Metric", "Before", "After prune channels", "After prune layers(final)"],
        ["mAP", {origin_model_metric[0][2]}, {compact_model_metric1[0][2]}, {compact_model_metric2[0][2]}],
        ["Parameters", {origin_nparameters}, {compact_nparameters1}, {compact_nparameters2}],
        ["Inference", {pruned_forward_time}, {compact_forward_time1}, {compact_forward_time2}]
    ]
   
    print(AsciiTable(metric_table).table)


    pruned_cfg_name = opt.cfg.replace('/', '/prune_{}_keep_{}_{}_shortcut_'.format(opt.global_percent,opt.layer_keep,opt.shortcuts))
    pruned_cfg_file = write_cfg(pruned_cfg_name, [model.hyperparams.copy()] + compact_module_defs)
    print('Config file has been saved:',pruned_cfg_file)

    compact_model_name = opt.weights.replace('/', '/prune_{}_keep_{}_{}_shortcut_'.format(opt.global_percent,opt.layer_keep,opt.shortcuts))

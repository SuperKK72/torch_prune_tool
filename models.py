...
def create_modules(module_defs, img_size, arc):
    ...
    for i, mdef in enumerate(module_defs):
        ...
        if mdef['type'] == 'convolutional':
            ...
            modules.add_module('Conv2d', MaskedConv2d(in_channels=output_filters[-1],
...
class Darknet(nn.Module):
    ...
    def set_masks(self, prune_idx, masks):
        # Should be a less manual way to set masks
        # Leave it for the future
        #Convidx2mask = {idx: mask.astype('float32') for idx, mask in zip(prune_idx, masks_list)}
        #size = self.module_list[i][0].weight.T.cpu().abs().detach().numpy().flatten()

        #for idx ,mask in zip(prune_idx)
        for idx in prune_idx:
            self.module_list[idx].Conv2d.set_mask(torch.from_numpy(masks[idx]))
